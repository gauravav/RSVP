...
function veventFor(event, stamp) {
  ...
  return [
    ...
    `DESCRIPTION:${escapeText(
      [event.timeLabel, event.note, event.venue, event.address].filter(Boolean).join('\n')
    )}`,
    'END:VEVENT',
  ];
}
...
// Android is where .ics is weakest: Chrome saves it to Downloads and leaves the
// guest to find an app that will open it. A Google Calendar template URL opens
// the app directly instead. It only carries one event, so multi-event guests
// get one link per ceremony.
export function googleCalendarUrl(event) {
  ...
  const params = new URLSearchParams({
    ...
    details: [event.timeLabel, event.note].filter(Boolean).join('\n'),
    location: `${event.venue}, ${event.address}`,
  });
  return `https://calendar.google.com/calendar/render?${params}`;
}

export function calendarFileUrl(eventKeys) {
  return `/api/calendar?events=${encodeURIComponent(eventKeys.join(','))}`;
}
